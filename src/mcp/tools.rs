...
impl TillerServer {
    ...
    ///
    /// This tool creates a new transaction in the local SQLite database and returns the primary key
    /// it was given. Changes are NOT automatically synced to the Google Sheet - call `sync_up` to
    ...
    /// - `transaction_id`: What Tiller calls this transaction — the value bound for the sheet's
    ///   `Transaction ID` column. It is data about the row, not the row's primary key; the primary
    ///   key is always generated by this tool. Omit it for a transaction Tiller has never seen, in
    ///   which case the generated `user-` key is written to the sheet as well and marks the row as
    ///   one Tiller did not create. Supply it when recording a transaction Tiller already knows
    ///   about, so the sheet keeps Tiller's own ID rather than carrying a `user-` row beside
    ///   Tiller's copy of the same transaction. It need not be unique.
    ...
    /// # Returns
    ///
    /// On success, returns a message indicating the transaction was inserted, along with the
    /// primary key the row was given.
    ///
    /// # Example
    ///
